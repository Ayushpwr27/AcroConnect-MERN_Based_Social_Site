...
const adminCtrl = {
  ...
  getTotalSpamPosts: async (req, res) => {
    try {
      ...
      const reportedPosts = await posts.filter(post => post.reports.length>=1);
  ...
  getSpamPosts: async (req, res) => {
    try {
      const posts = await Posts.find()
        ...
        .populate({ path: "user", select: "username avatar email images" });
  ...
  deleteUser: async (req, res) => {
    try {const user = await Users.findOneAndDelete({
        _id: req.params.id
      });

      await Posts.deleteMany({ user : { $in: user._id } });
